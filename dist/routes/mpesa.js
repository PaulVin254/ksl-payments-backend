...
mpesaRouter.post("/stkpush", async (req, res) => {
    try {
        ...
        logEvent("INFO", "Initiating STK push request", { fullName, phone: phoneNumber, amount, tier: paymentTier }, "STK_PUSH");
        ...
        if (numericAmount < 10000 && finalPaymentTier === "full") {
            logEvent("WARN", `Tier integrity protection: amount Ksh ${numericAmount} < 10,000 for requested 'full' tier. Automatically downgraded to 'deposit'.`, { fullName, cleanPhone, amount: numericAmount }, "FINANCIAL_INTEGRITY");
 ...
 */
async function safeUpdatePayment(checkoutRequestId, payload) {
 ...
 */
mpesaRouter.post("/clear-cooldown", (req, res) => {
    try {
...
mpesaRouter.post("/callback", async (req, res) => {
    ...
    if (isProd && !configuredSecret) {
        logEvent("ERROR", "Critical Security Alert: DARAJA_WEBHOOK_SECRET is not configured in production!", undefined, "SECURITY");
    ...
    if (!providedToken || providedToken !== expectedSecret) {
        logEvent("WARN", "Unauthorized Daraja webhook attempt: secret token mismatch or missing", { query: req.query, ip: req.ip }, "SECURITY");
    ...
    try {
        ...
        logEvent("INFO", `Daraja Callback received (ResultCode: ${ResultCode})`, { CheckoutRequestID, ResultDesc }, "CALLBACK");
        ...
        if (existingRecord?.status === "verified") {
            logEvent("INFO", `Idempotency Guard: Transaction ${CheckoutRequestID} is already verified. Skipping duplicate callback.`, undefined, "CALLBACK");
            return;
        }
        if (ResultCode === 0) {
            ...
            try {
                ...
                if (gatewayStatus && gatewayStatus.ResultCode !== undefined && String(gatewayStatus.ResultCode) !== "0") {
                    logEvent("WARN", `Security Alert: Callback claimed success but STK Query returned ResultCode ${gatewayStatus.ResultCode}. Halting verification.`, gatewayStatus, "SECURITY");
                    return;
                }
            }
            catch (queryErr) {
            ...
            try {
                ...
                logEvent("SUCCESS", `Payment confirmed for ${updatedRecord?.full_name || "student"} (Receipt: ${MpesaReceiptNumber})`, { CheckoutRequestID, amount: parsedAmount, receipt: MpesaReceiptNumber }, "CALLBACK");
                if (updatedRecord) {
                ...
                if (updateErr.code === "23505" || updateErr.message?.includes("idx_unique_mpesa_receipt") || updateErr.message?.includes("unique")) {
                    logEvent("ERROR", `REPLAY ATTACK BLOCKED: Duplicate M-Pesa receipt ${MpesaReceiptNumber} rejected by database`, updateErr.message, "SECURITY");
                    return;
                }
                throw updateErr;
            }
            ...
            logEvent("WARN", `Payment unconfirmed (${mappedError.category}: Code ${ResultCode})`, { CheckoutRequestID, failureReason: mappedError.failureReason }, "CALLBACK");
...
mpesaRouter.get("/status/:checkoutRequestId", async (req, res) => {
    try {
        ...
        if (data.status === "pending" && data.created_at) {
            ...
            if (ageInSeconds > 25) {
                logEvent("INFO", `Active reconciliation check for ${checkoutRequestId} (Age: ${Math.round(ageInSeconds)}s)`, undefined, "RECONCILIATION");
