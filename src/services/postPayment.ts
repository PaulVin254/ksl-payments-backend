 ...
 */
export async function handlePaymentSuccess(record: ConfirmedPaymentRecord): Promise<void> {
  const groupLink =
    process.env.WHATSAPP_CLASS_GROUP_LINK || "https://chat.whatsapp.com/JHAPRzElBgQIUhwjHfxkP8";

  logEvent(
    "INFO",
    `Starting post-payment automations for ${record.full_name} (Receipt: ${record.mpesa_receipt})`,
    {
  ...
  if (!isWhatsAppEnabled) {
    logEvent(
      "INFO",
      "WhatsApp notification skipped (ENABLE_WHATSAPP is false or not set)",
      undefined,
      "WHATSAPP"
    );
    return;
