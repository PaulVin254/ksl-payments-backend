...
export async function runReconciliationSweep(): Promise<void> {
  ...
  try {
    ...
    for (const row of pendingRows) {
      if (!row.checkout_request_id || !row.created_at) continue;
      ...
      if (ageInSeconds >= 60 && ageInSeconds <= 1800) {
        logEvent(
          "INFO",
          `Reconciler checking pending transaction ${row.checkout_request_id} (Age: ${Math.round(ageInSeconds)}s)`,
          undefined,
          "RECONCILER"
        );
        ...
        try {
          ...
          if (queryRes && queryRes.ResultCode !== undefined) {
            ...
            if (resCode === "0") {
              logEvent(
                "SUCCESS",
                `Reconciler verified payment for ${row.full_name} (${row.checkout_request_id})`,
                queryRes,
                "RECONCILER"
              );
              ...
              if (updated) {
                handlePaymentSuccess({
                  ...
                  logEvent("ERROR", "Reconciler automation error", err.message, "RECONCILER")
                );
              }
              ...
              logEvent(
                "WARN",
                `Reconciler confirmed failure (${mapped.category}) for ${row.checkout_request_id}`,
                queryRes,
                "RECONCILER"
              );
      ...
      // 2. If older than 30 minutes without confirmation: mark expired
        ...
        logEvent(
          "INFO",
          `Auto-expiring stale pending payment for ${row.full_name} (${row.checkout_request_id})`,
          undefined,
          "RECONCILER"
        );
 ...
 */
export function startReconciliationWorker(intervalMs = 120 * 1000): void {
